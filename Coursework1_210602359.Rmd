...
We can plot the Time Series and We can also use the function `plot(decompose())` to plot the components of the Time Series in order to visualise it. Again, these are the trend, the seasonality and the error.
...
From the plot of the Time Series wan see that there is no obvious change in variance with time so it seems the Time Series is Homoskedastic.
From the decompose plot, we caan see that there is an overall decreasing trend. We can also see a pattern in the seasonality; there is an increase towards  the end of the year. Later on, we will see the same thing using Prophet. From the residual error plot, we can see that there is no obvious trend or change in variance so it seems like it is stationary.

## 1.4 Forecasting

Now that we have formatted the data properly we can use prophet to forecast. To do this we need to make a new data frame of how much time we wish to forecast (predict). We will be forecasting one year and since we have monthly time periods so we will do monthly frequency and 12 periods.
